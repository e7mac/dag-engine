const Runner = {
  async execute() {
    ...
    try {
      const { run_id } = await API.fetch(`/workflows/${State.selectedWorkflowId}/run`, {
        method: 'POST',
  ...
  poll(runId) {
    ...
    State.pollTimer = setInterval(async () => {
      ...
      try {
        ...
        if (idx >= 0) State.runs[idx] = run; else State.runs.push(run);
        Sidebar.renderRunList();
        if (run.status === 'completed' || run.status === 'failed') {
          clearInterval(State.pollTimer);
          State.pollTimer = null;
          Sidebar.selectRun(runId);
          UI.toast(`Run ${run.status}`, run.status === 'completed' ? 'success' : 'error');
          if (State.statsVisible) API.fetchStats();
        }
      } catch (e) {
        if (attempts > 30) { clearInterval(State.pollTimer); State.pollTimer = null; }
      }
    }, 500);
  },
