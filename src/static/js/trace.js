const Trace = {
  clear() {
    document.getElementById('trace-list').innerHTML =
      '<div class="empty-state" style="font-size:12px;padding:20px">' +
  ...
  render(trace, fullRun = null) {
    ...
    if (!trace.nodes || trace.nodes.length === 0) {
      el.innerHTML =
        '<div class="empty-state" style="font-size:12px;padding:20px">No trace data</div>';
      return;
    }
    ...
    let resumeBtn = '';
    if (fullRun && fullRun.status === 'failed') {
      resumeBtn =
        '<div style="padding:8px 12px;border-bottom:1px solid var(--border)">' +
          `<button class="primary sm" onclick="Runner.resume('${fullRun.run_id}')">` +
            'Resume from failure' +
          '</button>' +
        '</div>';
    }

    // Build trace node HTML
    const traceHtml = trace.nodes.map((n, i) => {
      const isLast = i === trace.nodes.length - 1;
      const dur = n.duration_ms != null ? n.duration_ms.toFixed(0) + 'ms' : '';
      const branch = n.branch_taken
        ? `<span class="branch-badge">${UI.esc(n.branch_taken)}</span>`
        : '';
      const attempts = n.attempts > 1 ? ` | ${n.attempts} attempts` : '';
      const startTime = n.started_at
        ? n.started_at.replace(/^.*T/, '').replace(/\+.*$/, '')
        : '';

      // Build expandable detail sections
      ...
      if (nr) {
        ...
        if (sections.length > 0) {
          ...
          const sectionHtml = sections.map(s => {
            const json = typeof s.data === 'string'
              ? s.data
              : JSON.stringify(s.data, null, 2);
            return (
              '<div class="detail-section">' +
                `<span class="detail-label">${s.label}</span>` +
                `<pre class="detail-json">${UI.esc(json)}</pre>` +
              '</div>'
            );
          }).join('');

          details =
            `<div class="trace-toggle" onclick="UI.toggleDetail('${uid}')">Show details</div>` +
            `<div class="trace-detail" id="${uid}" style="display:none">${sectionHtml}</div>`;
        }
      }
      ...
      let meta = n.status;
      if (dur)       meta += ' | ' + dur;
      if (attempts)  meta += attempts;
      if (startTime) meta += ' | ' + startTime;

      return (
        `<div class="trace-node" data-node-id="${UI.esc(n.node_id)}">` +
          '<div class="trace-line">' +
            `<div class="trace-dot ${n.status}"></div>` +
            (isLast ? '' : '<div class="trace-connector"></div>') +
          '</div>' +
    ...
    el.innerHTML = resumeBtn + traceHtml;

    // Attach hover handlers for DAG highlighting
    ...
    let html = `
      ...
      <div class="stat-card">
        ...
        <div class="stat-value">
          ${data.latency.avg_ms.toFixed(0)}<span style="font-size:12px;font-weight:400">ms</span>
        </div>
        <div class="stat-sub">
          p95 ${data.latency.p95_ms.toFixed(0)}ms / max ${data.latency.max_ms.toFixed(0)}ms
        </div>
      </div>
      <div class="stat-card">
        ...
        <div class="stat-sub">
          ${data.totals.nodes_succeeded} nodes ok / ${data.totals.nodes_failed} failed
        </div>
      </div>`;
